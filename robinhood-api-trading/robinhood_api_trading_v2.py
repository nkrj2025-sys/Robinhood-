...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Estimate a Robinhood crypto order by default. Live placement requires "
            "an account number, ROBINHOOD_PLACE_REAL_ORDER=true, --place, and an "
    ...
    parser.add_argument(
        "--asset-quantity",
        help="Asset quantity to trade. Defaults to 0.000001 unless --quote-amount is set.",
    )
    parser.add_argument(
        "--quote-amount",
    ...
    if not args.asset_quantity and not args.quote_amount:
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    print("Order preview:")
    print(json.dumps(order_preview, indent=2))

    if args.asset_quantity:
        estimated_price = api_trading_client.get_estimated_price(
            symbol=args.symbol,
            side="ask" if args.side == "buy" else "bid",
            quantity=args.asset_quantity,
        )
        ...
        print("Best bid/ask:")
        print(json.dumps(best_bid_ask, indent=2))
