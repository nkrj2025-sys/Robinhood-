...
def find_trading_pair(
    trading_pairs: List[Dict[str, Any]], symbol: str
) -> Dict[str, Any]:
    for trading_pair in trading_pairs:
        if trading_pair.get("symbol") == symbol:
            return trading_pair
    raise RuntimeError(f"Robinhood did not return trading pair data for {symbol}.")
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    print(f"Loaded trading pair: {trading_pair.get('symbol', args.symbol)}")
