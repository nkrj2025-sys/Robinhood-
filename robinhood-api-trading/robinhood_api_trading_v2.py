...
def require_requested_trading_pair(
    trading_pairs: List[Dict[str, Any]], requested_symbol: str
) -> Dict[str, Any]:
    for trading_pair in trading_pairs:
        if trading_pair.get("symbol") == requested_symbol:
            return trading_pair

    raise RuntimeError(
        f"Robinhood did not return the requested trading pair: {requested_symbol}"
    )
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    trading_pair = require_requested_trading_pair(trading_pairs, args.symbol)
